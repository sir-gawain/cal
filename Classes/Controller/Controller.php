...
use TYPO3\CMS\Cal\Controller\ViewController;
...
class Controller extends \TYPO3\CMS\Frontend\Plugin\AbstractPlugin {
	...
	public function initConfigs() {
		...
		while (!Calc::isValidDate($this->getDateTimeObject->day, $this->getDateTimeObject->month, $this->getDateTimeObject->year)) {
...
// 		$this->conf ['view.'] ['category'] = implode (',', $catIDs);
		...
		if ($rightsObj->isLoggedIn()) {
			...
			$result = $connection->select(['tx_cal_calendar_subscription'], 'fe_users', ['uid' => $rightsObj->getUserId()]);
	...
	public function searchEvent() {
		...
		if (!$start_day) {
			...
			$start_day = new CalDate (\TYPO3\CMS\Cal\Utility\Functions::getYmdFromDateString($this->conf, $start_day).'000000');
			$start_day->setHour(0);
		...
		if (!$end_day) {
			...
			$end_day = new CalDate (\TYPO3\CMS\Cal\Utility\Functions::getYmdFromDateString($this->conf, $end_day).'000000');
			$end_day->setHour(23);
		...
		if ($this->piVars ['single_date']) {
			$start_day = new CalDate (\TYPO3\CMS\Cal\Utility\Functions::getYmdFromDateString($this->conf, $this->piVars ['single_date']));
			$start_day->setHour(0);
			$start_day->setMinute(0);
			$start_day->setSecond(0);
			$start_day->setTZbyId('UTC');
	...
	public function searchUserAndGroup() {
		...
		if ($searchword != '') {
			$builder->andWhere($this->cObj->searchWhere($searchword, $this->conf ['view.'] ['search.'] ['searchUserFieldList'], 'fe_users'));
		}
		...
		if ($searchword != '') {
			$builder->andWhere($this->cObj->searchWhere($searchword, $this->conf ['view.'] ['search.'] ['searchGroupFieldList'], 'fe_groups'));
		}
		...
		if (!$this->piVars ['getdate']) {
			if ($this->piVars ['week']) {
				...
				if ($this->piVars ['month'] == 2) {
					...
					if (in_array($this->piVars ['month'], array(
							4,
							6,
							9,
							11
	 ...
	 *
	 */
	public function getParametersForTyposcriptLink(&$parameterArray, $overrulePIvars = array(), $cache = false,
		$clearAnyway = false, $altPageId = 0
	) {
